...
class ProductController extends Controller
{
    ...
    public function edit(Product $product)
    {
        ...
        if ($product->user_id !== Auth::id()) {
            Log::warning('Unauthorized edit attempt', ['user_id' => Auth::id(), 'product_id' => $product->id]);
    ...
    public function update(Request $request, Product $product)
    {
        ...
        if ($product->user_id !== Auth::id()) {
            Log::warning('Unauthorized update attempt', ['user_id' => Auth::id(), 'product_id' => $product->id]);
        ...
        try {
            ...
            Log::info('Validation passed', ['product_id' => $product->id, 'validated_data' => $validated]);
            ...
            Log::error('Validation failed', ['product_id' => $product->id, 'errors' => $e->errors()]);
            return back()->withErrors($e->errors())->withInput();
        } catch (\Exception $e) {
            Log::error('Error updating product', ['product_id' => $product->id, 'error' => $e->getMessage()]);
            return back()->withErrors(['update_error' => 'An error occurred while updating the product: ' . $e->getMessage()])->withInput();
        }
    }
